...
namespace CMPH_BugTracker.Controllers
{
    public class ProjectsController : Controller
    {
        ...
        {
            IQueryable<Project> result = null; if (searchStr != null) { result = db.Projects.AsQueryable();
                result = result.Where(p => p.Title.Contains(searchStr) 
                || p.Body.Contains(searchStr) 
                || p.Tickets.Any(c => c.Body.Contains(searchStr)));
            }
            else { result = db.Projects.AsQueryable(); }
